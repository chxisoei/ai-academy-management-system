...
const Report = () => {
  ...
  const handleGenerate = () => {
    ...
    const className = studentInfo?.classes?.name ? `${studentInfo.classes.name} ` : '';
    ...
    const cleanKeywords = keywords ? `선생님 의견인 '${keywords}' 사항을 반영하여, ` : '';

    // Simulate AI generation with custom profiles
    setTimeout(() => {
      setGenerating(false);
      
      let generatedContent = '';
      if (pType === '분석형') {
        generatedContent = `${parentTitle}, 안녕하세요. ${className}${name}이 담당 수학 강사 김원장입니다.
...
[세부 단원 및 약점 유형 진단]
${cleanKeywords}${name}이는 기본 개념의 완성도는 양호하나, 오답 분석 결과 도형을 응용한 다항식 연산 및 활용 유형에서 식 세우는 과정에 미세한 약점이 포착되었습니다. 제한된 시간 내 풀이하는 시간 분배 훈련이 핵심 과제입니다.
...
자세한 오답 정량 분석 표와 단원 처방지는 별도 인쇄물로도 송부해 드리겠습니다. 감사합니다!`;
        ...
        generatedContent = `${parentTitle}, 안녕하세요. ${className}${name}이 담당 강사 김원장입니다. 
...
[향후 목표 및 실행 방안]
${cleanKeywords}이번 결과를 토대로 다음 시험의 구체적 목표와 액션플랜을 설정하였습니다.
...
목표한 성과를 확실하게 달성할 수 있도록 빈틈없이 이끌겠습니다. 믿고 맡겨주셔서 감사합니다.`;
        ...
        generatedContent = `${parentTitle}, 안녕하세요! ${className}${name}이와 소통하며 지도하는 김원장입니다.
이번 ${examType} 성취 결과를 토대로, 학원과 가정이 연계하여 시너지를 낼 수 있는 학습 가이드를 드립니다.

- 이번 시험 점수: ${currentScore}점
- 평가 코멘트: ${scoreComparison}

${cleanKeywords}학원에서 관찰한 결과, ${name}이는 학원 수업 시 집중력이 아주 훌륭하지만 가정에서의 복습과 끝까지 스스로 식을 완성하는 자습 관리가 추가된다면 훨씬 빠른 성장이 가능합니다.
...
궁금하신 점이 있으시면 언제든 편하게 연락해주세요!`;
        ...
        generatedContent = `${parentTitle}, 안녕하세요! 우리 ${className}${name}이를 아끼고 정성껏 지도하는 김원장입니다. 
오늘 ${name}이가 정말 고생 많았던 ${examType}에 대한 보람찬 격려 소식을 전해드립니다.

- 이번 시험 점수: ${currentScore}점
- 노력의 성과: ${scoreComparison}

${cleanKeywords}${name}이는 이번 평가를 준비하며 단 한 번의 지각 없이 누구보다 진지한 태도로 성실하게 숙제와 식 쓰기 훈련에 임했습니다. 점수를 뛰어넘어 포기하지 않고 끝까지 성취하려 노력한 이 소중한 태도와 인성은 앞으로의 성장에 있어 가장 빛나는 자산이 될 것입니다.
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-3 gap-3" style={{ marginBottom: '1rem' }}>
            ...
            <div className="input-group" style={{ marginBottom: 0 }}>
              ...
              <input type="number" className="input-field" value={currentScore} onChange={e => setCurrentScore(Number(e.target.value))} style={{ fontSize: '0.8125rem' }} />
            </div>
            <div className="input-group" style={{ marginBottom: 0 }}>
              <label className="input-label">이전 점수</label>
              <input type="number" className="input-field" value={prevScore} onChange={e => setPrevScore(Number(e.target.value))} style={{ fontSize: '0.8125rem' }} />
            </div>
          </div>
